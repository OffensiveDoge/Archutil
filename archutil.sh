...
mainmenu() {
    while [[ "${menurunning}" = true ]]; do
        ...
        if [[ "${menukey}" = "q" ]]; then
            ...
            exec bash
            exit 0
        elif [[ ($menukey = "${downkey}" || $menukey = "${downkey2}") && $currentnum < 4 ]]; then #browser
...
while true; do
 ...
 #  DOWN BLOCK
        ...
        currentdoc="docgui${currentnum}"
        ${currentdoc}
    elif [[ ($key = "${downkey}" || $key = "${downkey2}") && $currentnum < 4 ]]; then
        (( currentnum += 1 ))
        currentdoc="docgui${currentnum}"
        ${currentdoc}
    fi
...
while [[ "${menurunning}" = true ]]; do
    ...
    if [[ "${optkey}" = "q" ]]; then
        ...
        exit 0
 #  DOWN BLOCK
    elif [[ ($optkey = "${downkey}" || $optkey = "${downkey2}") && $currentnum < 3 ]]; then #browser
        printf -v "optcursor$currentnum" '%s' " "
        printf -v "optcursor$((currentnum + 1))" '%s' "${white}>"
        (( currentnum += 1 ))
        optiongui
 #  UP BLOCK
    elif [[ ($optkey = "${upkey}" || $optkey = "${upkey2}") && $currentnum > 1 ]]; then
        printf -v "optcursor$currentnum" '%s' " "
        printf -v "optcursor$((currentnum - 1))" '%s' "${white}>"
        (( currentnum -= 1 ))
        optiongui
